...
COMMON_LOG_FORMAT %{CLIENT:client_ip} %{HTTPDUSER:ident} %{USER:auth} \[%{HTTPDATE:ts:ts-httpd}\] "(?:%{WORD:http_method:tag} %{NOTSPACE:request}(?: HTTP/%{NUMBER:http_version:float})?|%{DATA})" %{NUMBER:http_status_code:tag} (?:%{NUMBER:bytes:int}|-)
